...
const StyledContainer = styled.section`
  ...
  height: 100%;
  padding: ${props =>
    props.isConfirmation ? '19px 20px 10px 20px' : '30px 50px'};
...
const ContainerCreate = props => {
      ...
      <div key="transaction details">
        ...
        <div>
          <StyledConfirmationMessage>
            Please check the information below and confirm the transaction in
            MetaMask.
          </StyledConfirmationMessage>
          <StyledSeparator />
        </div>
        <StyledDetailsContainer>
          <StyledDetailTitle>Payee Address: {props.payee}</StyledDetailTitle>
          <StyledDetailTitle>Bill Amount: {props.billAmount}</StyledDetailTitle>
          {props.payers.map((payer, id) => (
            <div>
              <StyledDetailTitle>{`Payer #${id}`}</StyledDetailTitle>
  ...
  return (
    <StyledContainerWrapper>
      <StyledContainer isConfirmation={props.type === 'confirm'}>
        {toRender}
      </StyledContainer>
    </StyledContainerWrapper>
  );
